...
-type params() :: {Module::atom(), LeafLength::number()}.
...
-callback length(value()) -> number().
-callback cache(value()) -> cache().
% length(Value1) + length(Value2) = length(merge_values(Value1, Value2))
-callback merge_values(value(), value()) -> value().
% if {Value1, Value2} = split_value(Value) 
% then length(Value1) + length(Value2) = length(Value)
-callback split_value(value(), number()) -> {value(), value()}.
-callback merge_caches(cache(), cache()) -> cache().


% public
-spec leaf_rope(value(), params()) -> rope().

leaf_rope(Value, {Module, _}) ->
	Len = Module:length(Value),
...
split({0, Len, Leaf}, Pos, Params) ->
	{Val, _} = Leaf,
	{Module, _} = Params,
	{Val1, Val2} = Module:split_value(Val, Pos),
	Leaf1 = {Val1, Module:cache(Val1)},
	Leaf2 = {Val2, Module:cache(Val2)},
...
merge({0, Len1, Leaf1}, {0, Len2, Leaf2}, Params) ->
	{Val1, Cache1} = Leaf1,
	{Val2, Cache2} = Leaf2,
	{Module, LeafLength} = Params,
	case (Len1 + Len2 < 2 * LeafLength) of 
		true ->
			Leaf = {Module:merge_values(Val1, Val2), Module:merge_caches(Cache1, Cache2)},
			{0, Len1 + Len2, Leaf}; % tree with leaf
		false ->
			{NewVal1, NewVal2} = Module:split_value(Module:merge_values(Val1, Val2), LeafLength),
			NewCache1 = Module:cache(NewVal1),
			NewCache2 = Module:cache(NewVal2),
			NewLen1 = Module:length(NewVal1),
			NewLen2 = Module:length(NewVal2),
			{1, NewLen1 + NewLen2, % tree
				{NewLen1 + 1, % 2-node
					{0, NewLen1, {NewVal1, NewCache1}}, % tree with leaf
					{0, NewLen2, {NewVal2, NewCache2}}, % tree with leaf
					Module:merge_caches(NewCache1, NewCache2)}} 
	end;
...
merge(Tree1 = {H1, Len1, Node1}, Tree2 = {H2, Len2, Node2}, Params) when H1 == H2 ->
	...
	{Module, _} = Params,
	{H1 + 1, Len1 + Len2, {Len1, {H1, Len1, Node1}, {H2, Len2, Node2}, Module:merge_caches(Cache1, Cache2)}};
...
merge({H1, Len1, {Key1, Tree11, Tree12, Cache1}}, Tree2 = {H2, Len2, Node2}, Params) when H1 == H2 + 1 ->
	Cache2 = extract_tree_cache(Tree2),
	{Module, _} = Params,
	{H1, Len1 + Len2, {Key1, Len1, Tree11, Tree12, {H2, Len2, Node2}, Module:merge_caches(Cache1, Cache2)}};
% node + 2-node = 3-node
merge(Tree1 = {H1, Len1, Node1}, {H2, Len2, {Key2, Tree21, Tree22, Cache2}}, Params) when H1 + 1 == H2 ->
	Cache1 = extract_tree_cache(Tree1),
	{Module, _} = Params,
	{H2, Len1 + Len2, {Len1, Len1 + Key2, {H1, Len1, Node1}, Tree21, Tree22, Module:merge_caches(Cache1, Cache2)}};
% 3-node + small
merge({H1, Len1, {_, _, Tree11, Tree12, Tree13, Cache1}}, Tree2 = {H2, Len2, _}, Params) when H1 == H2 + 1 ->
	...
	{H1 + 1, Len1 + Len2, % tree
		{Len11 + Len12, % 2-node
			{H1, Len11 + Len12, {Len11, Tree11, Tree12, Module:merge_caches(Cache11, Cache12)}}, % tree with 2-node
			{H1, Len13 + Len2, {Len1, Tree13, Tree2, Module:merge_caches(Cache13, Cache2)}}, % tree with 2-node
			Module:merge_caches(Cache1, Cache2)}};
% small + 3-node
merge(Tree1 = {H1, Len1, _}, {H2, Len2, {_, _, Tree21, Tree22, Tree23, Cache2}}, Params) when H2 == H1 + 1 ->
	...
	{H2 + 1, Len1 + Len2, % tree
		{Len1 + Len21, % 2-node
			{H2, Len1 + Len21, {Len1, Tree1, Tree21, Module:merge_caches(Cache1, Cache21)}}, % tree with 2-node
			{H2, Len22 + Len23, {Len22, Tree22, Tree23, Module:merge_caches(Cache22, Cache23)}},
			Module:merge_caches(Cache1, Cache2)}}; % tree with 2-node
...
flatten({0, _, {Val, _}}, undefined, _) ->
	Val;
flatten({0, _, {Val, _}}, AccBinary, {Module, _}) ->
	Module:merge_values(AccBinary, Val);
